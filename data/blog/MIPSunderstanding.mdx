---
title: InCTF Nationals 2021 MIPSunderstanding writeup
date: 01-10-2022 6:26 PM
tags:
- stack
- shellcode
- mips
...
![](https://imgur.com/21z6F5S.png) I dont know mips assembly and i solved it without understanding mips :)) . It's an easy challenge. Lets open the binary in ghidra and analyse the binary. Well there's alot of junk code in it so ill straight up show you the `thiago` and `keita` function.
...
**NX is actually <span style={{ color: 'red' }}>disabled</span> inside qemu**. To enable NX they have to patch the qemu binary. Also use gdb-gef instead of pwndbg because pwndbg has alot of issues when it comes to weird architectures.
...
```python expl.py https://raw.githubusercontent.com/4n0nym4u5/CTF-Writeups/main/MIPSunderstanding/exploit.py expl.py
#!/usr/bin/env python3.9
# -*- coding: utf-8 -*-
